...
def main():
    if len(sys.argv) != 3:
        print('usage: python face_detect_blend.py IMG1 IMG2')
        sys.exit(1)
    ...
    for (x, y, w, h) in img1_faces:
        cv2.ellipse(mask, (x + w//2, y + h//2), (w//5 + w//5, h//4 + h//5), 0, 0, 360, (255, 255, 255), -1)
    ...
    blend = laplacian_blend.pyr_blend(img1, img2_shifted, mask)
    
    # Display the output
    window = 'Face blend image result'
    ...
    project2_util.label_and_wait_for_key(window, mask,
                                         'Mask (hit a key when done)')
    project2_util.label_and_wait_for_key(window, blend,
